...
export default function ProductCard({ product }: { product: Product }) {
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm flex flex-col gap-2 hover:shadow-lg transition-all group">
      <div className="relative aspect-square bg-[#E5E7EB] rounded-xl overflow-hidden">
        ...
        />
        <button className="absolute top-2 right-2 p-2 bg-white/80 rounded-full shadow-sm hover:bg-white text-gray-400 hover:text-red-500">
        ...
        <div className="mt-2">
          ...
          {product.harga_discount && (
            <p className="text-xs text-gray-400 line-through">
              Rp {product.harga_discount.toLocaleString('id-ID')}
            </p>
          )}
        </div>
      </div>
